...
async def start_grpc_server():
    ...
    query_pb2_grpc.add_KnowledgeQueryServiceServicer_to_server(
        KnowledgeQueryServicer(), grpc_server
    )
    ...
    certs_available = (
        settings.KNOWLEDGE_QUERY_SERVICE_CERT_PATH
        and Path(settings.KNOWLEDGE_QUERY_SERVICE_CERT_PATH).exists()
        and settings.KNOWLEDGE_QUERY_SERVICE_KEY_PATH
        and Path(settings.KNOWLEDGE_QUERY_SERVICE_KEY_PATH).exists()
    )

    if certs_available:
        logger.info(
            "Certificates found, initializing mTLS...", event_name="GRPC_MTLS_INIT"
        )
        try:
            ...
            root_ca = (
                Path(settings.GRPC_TLS_CA_PATH).read_bytes()
...
async def health_check():
    if await engine.check_health():
        return {
            "status": "healthy",
            "mode": "standalone" if not settings.GRPC_TLS_CA_PATH else "cluster",
        }
...
async def query_knowledge_base(request: QueryRequest):
    ...
    try:
        ...
        logger.info(
            "HTTP Query processed successfully",
            event_name="HTTP_QUERY_SUCCESS",
